...
/**
 * Parse an expense command or voice transcript into structured financial data
 * Supports English, Hindi, and Marathi commands.
 * @param {string} text - User command or voice transcript
 * @returns {Promise<{ type: string, category: string|null, amount: number, description: string, confidence: number }>}
 */
export const parseExpenseWithSarvam = async (text) => {
  ...
  const systemPrompt = `You are FinVoice, an expert financial transaction parser.
Extract the transaction details from the given text (English, Hindi, or Marathi).
Respond with ONLY a raw, valid JSON object and nothing else. Do NOT include markdown formatting or backticks.

JSON Schema:
{
  "type": "expense" | "savings" | "spending",
  "category": "food" | "medical" | "education" | "others" | null,
  "amount": number,
  "description": string
}
...
Rules:
1. "type":
   - "savings" if text mentions saving, depositing, putting money into bank (e.g. saved, बचत, जमा, सेव्ह). Category must be null.
   - "expense" if text mentions category expenses (food, medical, education, etc.).
   - "spending" if general expenditure without specific category. Category must be null.
2. "category":
   ...
   - "others" for utilities, bills, rent, misc.
   - null for savings or generic spending.
3. "amount": Extracted numeric amount as a positive number (strip ₹, Rs, rupees, रुपये, commas). If no amount found, return 0.
4. "description": A concise summary of the transaction.`;
